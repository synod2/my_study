...
def LED_enc(input,userkey,ksbits):
 ...
 # for i in range(0,16):
  ...
  print "LED:",LED
  print "RN:",RN
  AddKey(keyNibbles,0) #랜덤키 앞 절반 우선 계산 , 연산 1
  '''
  for i in range(0,4):
    for j in range(0,4):
      state[i][j] ^= keyBytes[(4*i+j+step*16)%(LED/4)]
  ...
  for i in range(0,RN/4):
    for j in range(0,4):
      AddConstants(i*4+j) #연산2
      SubCell() #연산 3
      ShiftRow()
      print "\nafter shift",state
      MixColumn()
      print "\nafter mix",state
    AddKey(keyNibbles, i+1) #랜덤키 뒤 절반 이후 계산 
...
def main():
     ...
     try:
       ...
       # raw_input("")
        ...
        if lazy == 0:
            ...
            print "K : ",k_data
            print Lazy_LED_enc(p_data, k_data, len(K)*8, lazy).encode('hex') 
  ...
  try:
   #test = ""
   #my_key = test[0:32].decode("hex")
  #print "K = ",K.encode("hex")
